...
Then kids, and sorting and returning any number of product was made in get_items_by_price 
and products() changed also

Then I did the product by id, but first turned data structure into dict. It was trivial to change get_items_by_price
and products() and kids() did not have to be changed

For the pagination I first implemented the optional offset in get_items_by_price. Then changed products() to support
?page and ?count

I have no experience in testing web apps so this Flask app is without tests for now. Obviously wrong!
...
def get_items_by_price(n=None, offset=0):
    sort_prod = sorted(data.values(), key=lambda x: x['price'])
    if n:
        return sort_prod[offset:offset+n]
    else:
        return sort_prod
